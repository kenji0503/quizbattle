...
if ($prefillTitle === '' && $param_c1 && $param_c2 && $param_qid) {
    $prefetchError = null;
...
<body class="space">
    <header class="space-header">推し問バトル – 宇宙ルーム作成</header>
    ...
    <main class="space-scene">
        ...
        <section class="card">
            ...
            <?php if ($created_urls): ?>
                ...
                <p><a class="join-url" href="<?= htmlspecialchars($created_urls['join'], ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener"><?= htmlspecialchars($created_urls['join'], ENT_QUOTES, 'UTF-8') ?></a></p>
                <p>
                ...
                <?php else: ?>
                    <div class="picked-view">
                        <span class="picked-chip" style="opacity:.7">（テーマ未選択）</span>
                    </div>
                <?php endif; ?>
                ...
                <form method="post" id="createForm">
                    ...
                    <div class="scope-box">
                        <div style="display:flex;align-items:center;gap:.6rem;flex-wrap:wrap">
                            <h3 style="margin:0;font-size:16px">出題ジャンルの選択（複数可）</h3>
                            <span class="scope-badge">チェックボックス</span>
                            <span class="scope-badge">カテゴリ→サブ→テーマ</span>
                        </div>
                        ...
                        <div style="margin-top:10px">
                            <strong>選択済み：</strong>
                            <button type="button" id="resetPickedBtn" class="space-button reset-button" style="margin-left:.5rem;">
                                リセット
                            </button>
                            <div id="chosen" style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.4rem"></div>
                        </div>
                    </div>

                    <button class="space-button" type="submit">作成</button>
                </form>
            <?php endif; ?>
        </section>
    </main>
    ...
    <script>
        ...
        function renderPicked() {
            ...
            picked.forEach((title, key) => {
                ...
                tag.textContent = `${key} ${title}`;
                tag.style.cssText = 'display:inline-block;padding:2px 8px;border-radius:999px;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.18);font-size:12px';
                tag.title = 'クリックで削除';
        ...
        async function loadC1() {
            ...
            js.data.forEach(row => {
                ...
                a.href = '#';
                a.textContent = `${row.cate1}: ${row.title}`;
        ...
        function highlightC1() {
            [...c1List.children].forEach(li => {
                li.style.fontWeight = (parseInt(li.dataset.c1, 10) === parseInt(selC1 || -1, 10)) ? '800' : 'normal';
                li.style.textDecoration = (parseInt(li.dataset.c1, 10) === parseInt(selC1 || -1, 10)) ? 'underline' : 'none';
            });
        ...
        async function loadC2() {
            ...
            js.data.forEach(row => {
                ...
                const a = document.createElement('a');
                a.href = '#';
                a.textContent = `${row.cate2}: ${row.title}`;
                a.addEventListener('click', (e) => {
                    e.preventDefault();
                    selC2 = row.cate2;
                    highlightC2();
        ...
        function highlightC2() {
            [...c2List.children].forEach(li => {
                li.style.fontWeight = (parseInt(li.dataset.c2, 10) === parseInt(selC2 || -1, 10)) ? '800' : 'normal';
                li.style.textDecoration = (parseInt(li.dataset.c2, 10) === parseInt(selC2 || -1, 10)) ? 'underline' : 'none';
            });
        }
